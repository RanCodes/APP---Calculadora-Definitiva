...
import { fetchLogistics, saveLogistics } from '../services/persistenceService';
...
const WeightManager: React.FC<WeightManagerProps> = ({ weights, onWeightsChange, rates, onRatesChange }) => {
  ...
  const loadLogisticsFromApi = async () => {
    setLoading(true);
    try {
      const data = await fetchLogistics();
      onWeightsChange(data.weights || []);
      onRatesChange(data.rates || []);
      setError(null);
    } catch (err: any) {
      setError(err.message || 'No se pudo cargar la base de logística');
    } finally {
      setLoading(false);
    }
  };
  ...
  const persistLogistics = async () => {
    setIsSaving(true);
    try {
      await saveLogistics({ weights, rates });
      setError(null);
      setLastSavedAt(new Date().toISOString());
      alert('✅ Datos guardados en el contenedor.');
    } catch (err: any) {
      setError(err.message || 'No se pudo guardar la base de logística');
    } finally {
      setIsSaving(false);
    }
  };
  ...
  return (
    <div className="animate-fade-in pb-20">
      <div className="mb-6 space-y-2">
        {loading && (
          <div className="flex items-center justify-between bg-blue-50 border border-blue-200 text-blue-700 px-4 py-3 rounded-2xl">
            <span className="text-sm font-bold">Cargando logística desde la API...</span>
            <div className="w-4 h-4 border-2 border-blue-500 border-t-transparent rounded-full animate-spin"></div>
          </div>
        )}
        {error && (
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 bg-rose-50 border border-rose-200 text-rose-700 px-4 py-3 rounded-2xl">
            <span className="text-sm font-bold">{error}</span>
            <button onClick={loadLogisticsFromApi} className="px-3 py-2 bg-rose-600 text-white rounded-xl text-xs font-black uppercase tracking-widest">Reintentar</button>
          </div>
        )}
        {lastSavedAt && (
          <div className="bg-emerald-50 border border-emerald-200 text-emerald-700 px-4 py-2 rounded-2xl text-[11px] font-black uppercase tracking-[0.2em]">Guardado en contenedor: {new Date(lastSavedAt).toLocaleString()}</div>
        )}
      </div>
        ...
        <div className="space-y-8">
          ...
          <div className="bg-slate-900 border border-slate-700 p-8 rounded-[2rem] shadow-2xl relative overflow-hidden group">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <button
                onClick={persistLogistics}
                disabled={isSaving || loading}
                className="flex-1 py-3 px-4 bg-emerald-600 hover:bg-emerald-500 disabled:bg-emerald-300 text-white rounded-xl font-black text-xs uppercase tracking-widest transition-all shadow-lg shadow-emerald-500/20 active:scale-95 flex items-center justify-center gap-2"
              >
                {isSaving ? (
                ...
                {isSaving ? 'Guardando...' : 'Guardar API'}
              </button>
              <button
